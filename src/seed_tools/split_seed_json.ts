...
import * as study_json from './study_json';
...
async function main(seedPath: string, outputDir: string) {
  ...
  const msg = VariationsSeed.fromJson(seedJson, { ignoreUnknownFields: false });

  const studies = new Map<string, any[]>();
  for (const study of msg.study) {
  ...
  for (const study of studies) {
    ...
    study_json.writeStudyArray(studyArray, studyFile);
  }
}

export function registerCommand(program: Command) {
  program
    .command('split_seed_json')

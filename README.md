...
1. Create/modify a study in `/studies`, follow the protobuf schema in [`src/proto/study.proto`](/src/proto/study.proto).
2. Run `npm run seed_tools lint`, resolve issues, optionally pass `-- --fix` to fix formatting issues.
3. Create a PR to `main` branch.
4. Verify that everything works as intended using PR instructions.

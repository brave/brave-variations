...
import { type ProcessedStudy, StudyPriority, processStudyList, priorityToDescription } from './study_classifier'
...
function getOverallPriority(studies: ProcessedStudy[]) : StudyPriority {
  ...
  for (const study of studies) { // TODO: use reduce?
...
export function makeSummary(oldSeed: proto.VariationsSeed,
  ...
  const checkKey = (key: string): void => {
    ...
    if (newPriority > oldPriority) {
      if (oldPriority < StudyPriority.STABLE_MIN) {
        ...
        item.description = 'A known exp now ' + priorityToDescription(newPriority) + `(was: ${priorityToDescription(oldPriority)})`;
      }
    } else if (newPriority < oldPriority) {
      if (newPriority < StudyPriority.STABLE_MIN) {
        item.short_description = 'Removed';
        item.description = `A known exp have been removed (was: ${priorityToDescription(oldPriority)})`
      } else {
...
export function summaryToText(summary: SummaryItem[]): string {
  // https://source.chromium.org/search?q="BASE_DECLARE_FEATURE(kScrollUnification)"&sq=&ss=chromium%2Fchromium%2Fsrc
